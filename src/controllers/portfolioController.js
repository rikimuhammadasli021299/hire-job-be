...
const postPorto = async (req, res) => {
  let { nama_aplikasi, link_repo, type } = req.body;
  ...
  if (!link_repo || !type || !nama_aplikasi) {
    return res.status(400).json({
      code: 400,
      message: 'nama applikasi, link repo and type is required',
    });
  }
  ...
  let data = { photo: imageUpload.secure_url, link_repo, type, nama_aplikasi };
...
const putPorto = async (req, res) => {
  ...
  let id_user = req.user.id_user;
  let { link_repo, type, nama_aplikasi } = req.body;
  ...
  let newData = {
    ...
    type: type || data.type,
    nama_aplikasi: nama_aplikasi || data.nama_aplikasi,
  };
